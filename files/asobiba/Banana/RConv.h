...
public:
	...
	//URLを文字列に変換 
	static string UrlEncode(const string & inUrl);
	//置換.
	...
	static int RConv::KansujiToNumber(const string & inStr);
	static string RConv::NumberToKansuji(int inNum);

	static string RConv::atok(const string & inStr);
	static string RConv::atos(const string & inStr);

	//SJIS の1バイト目だけを残すような制御文字アタックを空白を追加
	static string SJISDangerESC(const string inStr);

	static RStringList Explode(const string & inStr , char inSep = ',');
