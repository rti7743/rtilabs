...
void RBlowFish::Blowfish_encipher (DWORD *xl, DWORD *xr) 
{
	union AWORD  Xl, Xr ;
...
void 		RBlowFish::Blowfish_decipher (DWORD *xl, DWORD *xr) 
{
	union AWORD  Xl ;
	union AWORD  Xr ;
...
void 		RBlowFish::Initialize (BYTE * key, int keybytes) 
{
	int  		i, j ;
...
DWORD		RBlowFish::GetOutputLength (DWORD lInputLong) 
{
	DWORD 	lVal ;
...
DWORD		RBlowFish::Encode (BYTE * pInput, BYTE * pOutput, DWORD lSize) 
{
...
// output buffer can be the same, but be sure buffer length is even MOD 8.
void		RBlowFish::Decode (BYTE * pInput, BYTE * pOutput, DWORD lSize) 
{
	DWORD 	lCount ;
	BYTE	*pi, *po ;
	int		i ;
...
void RBlowFish::test()
{
	...
	string key = "kaho";
	bf.Initialize( (unsigned char *) key.c_str() , key.size() );

	string val = "kaho tan to asobou!";

	int outlen = bf.GetOutputLength(val.size() );
	unsigned char* out = new unsigned char[outlen];

	//暗号化
	bf.Encode( (unsigned char *)val.c_str() , out , val.size() );

	int outlen2 = bf.GetOutputLength( outlen );
	unsigned char* out2 = new unsigned char[outlen2];

	//元に戻す
	bf.Decode( out , out2 , outlen );

	string val2 = (char*)out2;
