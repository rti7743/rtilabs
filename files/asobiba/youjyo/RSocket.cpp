// RSocket.cpp: RSocket クラスのインプリメンテーション
//
//////////////////////////////////////////////////////////////////////
 ...
 * @return  
 */
RSocket::RSocket(SOCKET inNewSocket)
{
	Socket = inNewSocket;
	IsConnect = true;
}
 ...
 */
void RSocket::Open( int type, int protocol ) throw(RIOCannotOpenException)
{
 ...
 */
void RSocket::Connect(string inHost , int inPort)	throw(RIOException)
{
 ...
 * @throws			RIOException			接続できなかったとき
 */
void RSocket::Connect(const SOCKADDR_IN * inSai) throw(RIOException)
{
	ASSERT(Socket != INVALID_SOCKET);
 ...
 */
void RSocket::Bind(int inPort) throw(RIOException)
{
	ASSERT(Socket != INVALID_SOCKET);
 ...
 * @throws			RIOException		bind失敗
 */
void RSocket::Bind(const SOCKADDR_IN * inSai) throw(RIOException)
{
	//すでにbindしています。
	if (IsConnect)	return ;
 ...
 */
void RSocket::Listen(int inBacklog ) throw(RIOException)
{
	ASSERT(Socket != INVALID_SOCKET);
 ...
 */
RSocket* RSocket::Accept() throw(RIOException)
{
	ASSERT(Socket != INVALID_SOCKET);
 ...
 */
void RSocket::HoldTillReadyForRead(long inTimeOutS) throw(RIOException)
{
	timeval UseTimeVal;
	FD_SET	ReadFlg;
	//セレクトの戻り値
 ...
 * @throws	RIOException	タイムアウト.
 */
void RSocket::HoldTillReadyForWrite(long inTimeOutS) throw(RIOException)
{
	timeval UseTimeVal;
	FD_SET	WriteFlg;
 ...
 */
void RSocket::Close() throw(RIOCannotCloseException)
{
	if(Socket == INVALID_SOCKET) return ;
 ...
 * @throws	RIOException	タイムアウト.
 */
int RSocket::Send( const char* inBuffer ,int inBufferLen , int inFlags ,int inTimeoutS ) throw(RIOException,RIOCannotWriteException)
{
	...
	//データ送信.
	int ret = ::send(Socket , inBuffer ,inBufferLen  ,inFlags  );
	//異常事態が起きたら、脱出.
	if ( ret < 0 )
	{
 ...
 * @throws	RIOException	タイムアウト.
 */
int RSocket::Reserve( char* outBuffer ,int inBufferLen , int inFlags ,int inTimeoutS ) throw(RIOException,RIOCannotReadException)
{
	...
	//データ送信.
	int ret = ::recv(Socket , outBuffer ,inBufferLen  ,inFlags  );
	if (ret < 0)
	{
