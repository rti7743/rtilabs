...
void Recognition_JuliusPlus::OnOutputResultFile(Recog *recog)
{
	...
	//juliusの結果を plus 整形済みパータンとして再構築する.
	std::list<OncSentence*> allSentence;
	convertResultFile(recog, &allSentence);
	...
	for(auto it = allSentence.begin() ; it != allSentence.end() ; ++it)
	{
		...
		this->PoolMainWindow->SyncInvokeLog(std::string() + "# plus_sentence_score:" + num2str((*it)->plus_sentence_score) + " score:" + num2str((*it)->score),LOG_LEVEL_DEBUG);
		...
		this->DictationCheckString = Recognition_JuliusPlusRule::converPress(yomi.str());
		break;
	}
	std::cout << std::flush;
