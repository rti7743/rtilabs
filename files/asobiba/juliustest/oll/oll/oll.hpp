...
namespace oll_tool{  
  ...
  int trainFile(const char* trainfile, const char* modelfile, 
		const trainMethod tm, const float C, const float bias, const int iter, bool verb);
  ...
  public:
    ...
    int trainFile(const T& a, const char* filename, 
		  const int iter = 10,  const bool verb = true);
  ...
  template<class T>
  int oll::trainFile(const T& a, const char* filename, const int iter, const bool verb){
