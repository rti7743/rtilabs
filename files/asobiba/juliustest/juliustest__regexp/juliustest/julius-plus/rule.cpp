...
bool rule::Commit(const std::string & parentRuleName ,std::ostringstream* grammar , std::ostringstream* voca  )
{
	...
	assert(!(wordonly == true && nestruleonly == true)); //共に only なのはどう考えてもオカシイ。
...
std::string rule::convertYomi(const std::string& word) const
{
	...
	boost::algorithm::replace_all(w , "パ","パ");
	boost::algorithm::replace_all(w , "ピ","ピ");
	boost::algorithm::replace_all(w , "プ","プ");
	boost::algorithm::replace_all(w , "ペ","ポ");
	boost::algorithm::replace_all(w , "ポ","ポ");
