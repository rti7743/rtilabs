 ...
 */
static void
cm_compute_from_nbest(StackDecode *sd, NODE *start, int stacknum, RecogProcess *r)
{
  NODE *node;
  ...
  if (sd->wordcm == NULL) {
    ...
    sd->wordcm = (LOGPROB *)myrealloc(sd->wordcm, sizeof(LOGPROB) * winfo->num);
    sd->wordnum = winfo->num;
  }
...
result_reorder_and_output(NODE **r_start, NODE **r_bottom, int *r_stacknum, int ncan, RecogProcess *r, HTK_Param *param)
{
  ...
  cm_compute_from_nbest(&(r->pass2), *r_start, *r_stacknum, r);
#endif
  num = 0;
