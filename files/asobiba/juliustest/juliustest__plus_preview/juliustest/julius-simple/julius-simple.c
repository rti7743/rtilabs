...
float get_sentnum_penalty(int sentnum)
{
	switch(sentnum)
	{
	case 0:
...
float get_cmscore_penalty(LOGPROB* confidence , int seqnum)
{
	int i;
	float avg = 0;
	for(i = 0 ; i < seqnum ; ++i)
...
output_result(Recog *recog, void *dummy)
{
  ...
  mseclen = (float)recog->mfcclist->param->samplenum * (float)recog->jconf->input.period * (float)recog->jconf->input.frameshift / 10000.0;
  printf("mseclen: %f\r\n" , mseclen);
  ...
  for(r=recog->process_list;r;r=r->next) {
	...
	printf(" (AM: %f  LM: %f)", s->score_am, s->score_lm);
      printf("\n");
      printf("score%d / msec : %f", n+1, s->score / mseclen);
      printf("\n");
      printf("score%d / msec * sentnum_penalty : %f", n+1, s->score / mseclen * get_sentnum_penalty(r->result.sentnum) );
      printf("\n");
      printf("score%d / msec * sentnum_penalty * get_cmscore_penalty: %f", n+1, s->score / mseclen * get_sentnum_penalty(r->result.sentnum) * get_cmscore_penalty(s->confidence,seqnum) );
      printf("\n");
