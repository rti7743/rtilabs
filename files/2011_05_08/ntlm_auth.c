/*
   ...
   Copyright (C) rti <super.rti@gmail.com> (http://twitter.com/#!/super_rti) 2011
...
/* rti7743 */
bool global_client_lanman_auth()
{
	/* Globals.bClientLanManAuth */
	return true;
}
typedef bool (*global_client_lanman_auth_def)();
global_client_lanman_auth_def lp_client_lanman_auth = global_client_lanman_auth;
...
#define ntlmssp_update gensec_ntlmssp_update
...
static struct wbcInterfaceDetails *init_interface_details(void)
{
	...
	if (!WBC_ERROR_IS_OK(wbc_status)) {
		DEBUG(0, ("could not obtain winbind interface "
...
static char winbind_separator(void)
{
	...
	if (!details) {
		DEBUG(0,("could not obtain winbind separator!\n"));
		return 0;
	}
	...
	if (!sep) {
		DEBUG(0,("winbind separator was NULL!\n"));
		return 0;
	}
...
*/
NSS_STATUS update_result_require_membership_of_sid_samba4bugfix(NSS_STATUS status , const char * username , bool stdout_diagnostics)
{
	...
	if (!require_membership_of_sid){
		/* no  require_membership_of_sid */
		return status;
	}
	
	/* username to sid */
	parse_wbinfo_domain_user(username, domain_name,
				 account_name);
	...
	if (stdout_diagnostics){
		...
		DEBUG(3,("user (%s) -> sid (%s) \n" ,username, sid_to_string_buffer));
	}
	...
	for (i = 0; i < num_sids; i++) {
		...
		if (stdout_diagnostics){
			DEBUG(3,("sid check reqsid(%s) vs groupsid (%s) \n" ,require_membership_of_sid, sid_to_string_buffer));
		}

		if (0 == strcmp(require_membership_of_sid , sid_to_string_buffer) )
		{
				 ...
				 bool stdout_diagnostics)
{
	...
	*/
	result = update_result_require_membership_of_sid_samba4bugfix(result,user , stdout_diagnostics);
	if (result == NSS_STATUS_NOTFOUND && response.data.auth.nt_status == 0){
				   ...
				   char **unix_name)
{
	...
	*/
	result = update_result_require_membership_of_sid_samba4bugfix(result,username , false);
...
static void offer_gss_spnego_mechs(void) {
	...
	myname_lower = talloc_strdup(ctx, get_winbind_netbios_name());
	if (!myname_lower) {
		return;
	}
...
static bool _spnego_parse_krb5_wrap(TALLOC_CTX *ctx, DATA_BLOB blob, DATA_BLOB *ticket, uint8_t tok_id[2])
{
	bool ret;
	ASN1_DATA *data;
	int data_remaining;
...
/*	dbf = x_stderr; */
	...
	pc = poptGetContext("wbinfo", argc, argv, long_options, 0);
	...
	if (opt_username) {
		...
		char *p = strchr_m(domain, winbind_separator());
		if (p) {
			opt_username = p+1;
			*p = '\0';
